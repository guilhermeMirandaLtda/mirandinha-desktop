...
| `view-studio` | **Studio — Editor Visual de Automações** | Duas telas dentro da mesma view: **galeria** (`#studio-gallery-view`, cards de exemplos, fluxos salvos e biblioteca da equipe) e **editor** (`#studio-editor-view`, canvas Drawflow + inspetor somente leitura + console próprio, com trace ao vivo pintando os blocos durante a execução). Ações: Validar, Salvar, Exportar, Publicar, Executar. **Ver a skill `studio` para tudo sobre este módulo.** |
...
2. **Catálogo Oficial (4 Robôs Nativos + fluxos publicados do Studio)**: desde o módulo Studio, `RPARunner.get_catalog()` devolve `AVAILABLE_JOBS + list_published_flows()`, e `execute_job_sync()` desvia `job_id` com prefixo `flow_` para o interpretador de grafos. Um fluxo publicado é indistinguível de um robô nativo nesta tela — ver a skill `studio`. Os quatro nativos são:
